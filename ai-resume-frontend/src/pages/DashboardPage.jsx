            ...
            <div className="card p-6 flex flex-col h-full">
              <div className="mb-6 flex items-center justify-between">
                <div>
                  ...
                  <h2 className="mt-1 text-md font-display font-bold text-white">Recent ATS Scores</h2>
                </div>
            ...
            <motion.div initial="hidden" animate="show" variants={containerVariants} className="card p-6 flex-1">
              ...
              <div className="space-y-4">
                {!hasData || stats.recentActivity.length === 0 ? (
                  <div className="text-sm text-slate-500 text-center py-8">No recent activity. Upload a resume to get started!</div>
                ) : (
                  stats.recentActivity.map((activity, i) => {
                    ...
                    >
                      ...
                      <div className="pb-1">
                        ...
                        <p className="mt-1 text-[10px] font-medium uppercase text-slate-500">{new Date(activity.time).toLocaleDateString()}</p>
                      </div>
                    </motion.div>
                  )})
                )}
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="card p-6 border-l-2 border-l-accent-violet flex flex-col justify-center relative overflow-hidden group">
            ...
            <p className="mt-4 text-sm leading-relaxed text-slate-400 relative z-10">
              {hasData ? (
                ...
                <><span className="text-white font-bold">Action required:</span> Please upload your first resume to receive AI-powered insights and actionable feedback.</>
              )}
            </p>
            <div className="mt-4 flex items-center justify-between relative z-10">
              <span className="text-xs text-slate-500">Generated just now</span>
              <Link to={resumes.length > 0 ? '/resume-boost' : '/resumes'} className="flex items-center text-xs font-bold text-accent-violet hover:text-accent-violet-dim transition-colors w-max">
                {resumes.length > 0 ? 'Fix with Resume Boost' : 'Upload Resume'} <ChevronRight className="ml-1 h-3.5 w-3.5" />
              </Link>
            </div>
          </motion.div>
